...
module.exports = function (yo) {
  ...
  module.generate = function (metadataPath, resourcesPath, targetPath, properties) {
    ...
    const metadata = archetypeMetadataModule(yo.fs.read(metadataPath));

    trace('Constructing archetype properties');
    const props = extend(metadata.getRequiredProperties(), properties);
    props.rootArtifactId = props.artifactId;

    trace('Copying top level pom');
    const pomPath = copyPom(resourcesPath, targetPath, '', props, yo.fs);

    trace('Processing top level FileSets');
    const filesets = metadata.getFileSets();
...
function copyPom (resourcesPath, targetPath, directory, properties, yoFs) {
  const fromPath = path.join(resourcesPath, directory, 'pom.xml');
  const targetDirectoryPath = path.join(targetPath, directory);
  const toPath = path.join(targetDirectoryPath, 'pom.xml');

  let copiedFiles = [];
  debug('Copying pom from', fromPath, 'to', toPath);
...
function processFunctionFactory (
  ...
  return function (fileContent, filePath) {
    let targetRelativeFilePath = path.relative(basePath, filePath);
    trace('Target relative file path: %s', targetRelativeFilePath);
    const filesetRelativeFilePath = path.relative(relativePath, targetRelativeFilePath);
    trace('Fileset relative file path %s', filesetRelativeFilePath);
    if (packaged) {
      trace('Repackaging');
      const postPackage = path.relative(relativePath, targetRelativeFilePath);
    ...
    if (filtered) {
      ...
      const strFileContent = (fileContent instanceof Buffer ? fileContent.toString() : fileContent);
      const newFileContent = fileFiltering.filter(strFileContent, properties);
      trace('Filtered');
...
function expandPathPlaceholders (targetPath, files, properties, yoFs) {
  trace('Expanding path placeholders');
