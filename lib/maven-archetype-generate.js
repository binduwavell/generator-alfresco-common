...
module.exports = function (yo) {
  ...
  module.generate = function (metadataPath, resourcesPath, targetPath, properties) {
    ...
    copyPom(resourcesPath, targetPath, '', properties, yo.fs);
    ...
    processModules(resourcesPath, targetPath, '', modules, properties, yo.fs);
  };
...
function copyPom (resourcesPath, targetPath, directory, properties, yoFs) {
  ...
  yoFs.copy(fromPath, toPath, {
    process:
      processFunctionFactory(resourcesPath, relativePath, '', copiedFiles, true, false),
  });

  expandPathPlaceholders(targetPath, copiedFiles, properties, yoFs);
}
      ...
      includeGlobs.forEach(includeGlob => {
        ...
        yoFs.copy(includeGlob, toPath, {
          ...
          process: processFunctionFactory(
            ...
            fileset.filtered, fileset.packaged),
        });
      });
    }
 ...
 * @param yoFs
 */
function processModules (resourcesPath, targetPath, directory, modules, properties, yoFs) {
  modules.forEach(module => {
    ...
    copyPom(resourcesPath, targetPath, modulePath, properties, yoFs);
    // TODO(bwavell): Do we need to add logic to update the artifactId or module name in the pom?
    processFileSets(resourcesPath, targetPath, modulePath, module.fileSets, properties, yoFs);
    processModules(resourcesPath, targetPath, modulePath, module.modules, properties, yoFs);
    // TODO(bwavell): Add logic to add modules to parent pom
  });
}
 ...
 * @returns {Function}
 */
function processFunctionFactory (basePath, relativePath, packagedPath, list, filtered, packaged) {
  // TODO(bwavell): Add filtering logic
 ...
 *
 * @param targetPath the root location we are generating into
 * @param files a list of files relative to the target path
 * @param properties list of properties that should be considered for expansion
 * @param yoFs
 */
