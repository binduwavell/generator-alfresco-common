...
module.exports = function (pomString) {
  ...
  module.findPlugin = function (first, second) {
    ...
    if (artifactId && plugins) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        var groupIdContent = (groupIdNode ? groupIdNode.textContent : undefined);
        var artifactIdContent = (artifactIdNode ? artifactIdNode.textContent : undefined);
        console.log(`CHECKING ${groupIdContent}:${artifactIdContent}`);

        // Don't bother matching things that don't at least match artifact Ids
        if (artifactId === artifactIdContent) {
          if (groupId) {
            if (groupId === groupIdContent) {
              console.log(`FOUND ${groupId}:${artifactId}`);
              return plugin;
            }
          } else {
            console.log(`FOUND ${artifactId}`);
            return plugin;
          }
        }
      }
  ...
  module.addPlugin = function (groupId, artifactId, version) {
    var plugin = module.findPlugin(groupId, artifactId);
    if (!plugin) {
      var plugins = domutils.getFirstNodeMatchingXPath('/pom:project/pom:build/pom:plugins', doc);
      if (plugins) {
        plugin = domutils.createChild(plugins, 'pom', 'plugin');
      }
    }
    if (plugin) {
      var groupIdNode = domutils.getOrCreateChild(plugin, 'pom', 'groupId');
      var artifactIdNode = domutils.getOrCreateChild(plugin, 'pom', 'artifactId');
      var versionNode = domutils.getOrCreateChild(plugin, 'pom', 'version');
      groupIdNode.textContent = groupId;
      artifactIdNode.textContent = artifactId;
      versionNode.textContent = version;
    }
