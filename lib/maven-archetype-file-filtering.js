'use strict';
var debug = require('debug')('generator-alfresco-common:maven-archetype-file-filtering');
...
function expandReference (text, reference, value) {
  ...
  var newText = text.replace(shorthandRe, (str, one, two) => one + value + two);
  newText = newText.replace(formalRe, value);

  if (text !== newText) {
    debug('FILTERED > ' + text.trim() + ' >> ' + newText.trim());
  }
 ...
 *
 * @param {!Array<string>} lines are processed one at a time
 * @returns {!Array<string>} original lines, with #set directives removed and associated
 *     references replaced
 */
function replaceSymbols (lines) {
  ...
  lines.forEach((line) => {
    ...
    if (match) {
      debug('CAPTURING SYMBOL > ' + match[1] + ' = ' + match[3]);
