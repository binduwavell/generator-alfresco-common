...
module.exports = {
  ...
  resolver: {
    mappings: {
      'desc': 'http://maven.apache.org/plugins/maven-archetype-plugin/archetype-descriptor/1.0.0',
  ...
  /**
   * Given a node, find all children with a given namespace and tag. Returns an empty list if the child is not found.
   *
   * @param {!Node} node
   * @param {string} ns
   * @param {string} tag
   * @returns {[Element]}
   */
  getChildren: function (node, ns, tag) {
    if (!node || !ns || !tag) throw new Error('All parameters to xml-dom-utils:getChildren() are required');
    var children = this.selectMatchingXPath(ns + ':' + tag, node);
    return children;
  },
   ...
   * starting from the provided document or element. Returns undefined if not
   * match is located.
   *
   * @param {string} xpath
   ...
   * @returns {(string|undefined)}
     */
  prettyPrint: function (docOrElement) {
    return pd.xml(new xmldom.XMLSerializer().serializeToString(docOrElement));
  },
