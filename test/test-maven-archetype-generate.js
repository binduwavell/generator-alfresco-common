...
describe('generator-alfresco-common:maven-archetype-generate', function () {
  ...
  describe('.generate()', function () {
    const tempDir = path.join(os.tmpdir(), 'generated-project');
  ...
  describe('.generate(groupId with several dots)', function () {
    const tempDir = path.join(os.tmpdir(), 'generated-deep-project');
    ...
    before(function (/* done */) {
      ...
      var archetypeGenerator = require('../index').maven_archetype_generate(yomock);
      var properties = {
        'groupId': 'my.deep.groupid',
        'artifactId': 'myartifactid',
        'version': '1.0-SNAPSHOT',
        'package': 'my.deep.package',
      };
      ...
      // console.log('LOOKING IN TEMP DIR', tempDir);
      // fs.commit(done);
    });
    ...
    it('copies files that are packaged', function () {
      [
        'myartifactid-repo-amp/src/main/java/my/deep/package/demoamp/Demo.java',
      ].forEach(file => {
        assert.ok(memFsUtils.existsInMemory(yomock.fs, path.join(tempDir, file)));
      });
    });
    ...
    it('filters pom.xml files', function () {
      ...
      assertFileIncludes(tempDir, 'pom.xml', [
        '<groupId>my.deep.groupid</groupId>',
        '<artifactId>myartifactid</artifactId>',
        '<version>1.0-SNAPSHOT</version>',
      ]);
    });
    ...
    it('filtering happens when paths in fileset don\'t have __placeholders__', function () {
      ...
      assertFileIncludes(tempDir, '/repo/src/main/resources/alfresco/extension/dev-log4j.properties',
        'log4j.logger.my.deep.package.demoamp.DemoComponent=${app.log.root.level}'
      );
    });

    it('filtering happens when paths in fileset have __placeholders__', function () {
      assertFileDoesNotInclude(tempDir, '/myartifactid-repo-amp/src/main/java/my/deep/package/demoamp/Demo.java',
        'package ${package}.demoamp;'
      );

      assertFileIncludes(tempDir, '/myartifactid-repo-amp/src/main/java/my/deep/package/demoamp/Demo.java',
        'package my.deep.package.demoamp;'
      );
    });
