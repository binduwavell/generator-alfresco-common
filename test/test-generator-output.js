...
describe('generator-alfresco-common:generator-output', function () {
  ...
  it('can provide info', function () {
    ...
    out.info('HELLO');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'INFO: HELLO');
  });

  it('can warn or issues', function () {
    var out = require('../index').generator_output(this.logmock);
    out.warn('WORLD');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'WARN: WORLD');
  });

  it('can error out', function () {
    var out = require('../index').generator_output(this.logmock);
    out.error('OBLITERATION!');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'ERROR: OBLITERATION!');
  });

  it('does not display anything when docs is called without arguments', function () {
    var out = require('../index').generator_output(this.logmock);
    out.docs();
    assert.strictEqual(this.logmock.get(), '');
  });

  it('can display documentation without a link', function () {
    var out = require('../index').generator_output(this.logmock);
    out.docs('useful info');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'useful info');
  });

  it('can display a link without documentation', function () {
    var out = require('../index').generator_output(this.logmock);
    out.docs(undefined, 'link');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'See: link');
  });
  ...
  it('can display multi-line documentation', function () {
    var out = require('../index').generator_output(this.logmock);
    out.docs(['useful', 'info']);
    assert.strictEqual(stripAnsi(this.logmock.get()), 'useful\n\ninfo');
  });

  it('can provide a definition', function () {
    var out = require('../index').generator_output(this.logmock);
    out.definition('example', 'this is an example.');
    assert.strictEqual(stripAnsi(this.logmock.get()), 'example: this is an example.');
  });

  it('can produce a color banner', function () {
    var out = require('../index').generator_output(this.logmock);
    assert.strictEqual(stripAnsi(out.bannerText()), out.rawBannerText());
  });

  it('can produce a fancy banner', function () {
    var out = require('../index').generator_output(this.logmock);
    out.banner('    ', 120);
    assert.strictEqual(stripAnsi(this.logmock.get()), out.rawBannerText());
  });

  it('can produce a fancy logo', function () {
    var out = require('../index').generator_output(this.logmock);
    out.banner('    ', 100);
    assert.strictEqual(stripAnsi(this.logmock.get()), out.rawLogoText());
  });
});
